...
# verify that all the bams have beds
    bed_count=$(find "intermediates/$sample" -type f -name "*.bed" | wc -l)
    # don't count any "_merged.bam" files we may have previously created
    bam_count=$(find "outputs/$sample" -type f -name "*.bam" ! -name "*_merged.bam" | wc -l)

    if [[ "$bed_count" -ne "$bam_count" ]]; then
        echo "ERROR: $sample is missing some BAM files." >&2
        exit 1
    else 
        echo "All intermediate bams created. Merging..."
    fi
done
