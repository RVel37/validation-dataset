...
# pull the GT field from each sample column (but if they are 0 then return empty line)
...
### ADD LOGIC FOR DUOS/SINGLETONS???? (Is this more appropriate than leaving other parental sample blank? It might be more accurate but won't be able to account for de novo)
...
### TO BE ADDED
    ...
    echo "Could not find annotation file for $FOLDERNO" >&2 # send to stderr
    continue
fi

# for easier logging/debugging:
# output of log.txt piped into either success.txt (one line for every success "sample X") or if log.txt doesn't have every expected line (wc -l), pipe output into stderr 
# log.txt wiped after every sample
