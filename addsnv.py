...
def mut(base, altbase):
    ...
    if base not in bases or (altbase is not None and altbase not in ['A', 'T', 'G', 'C']):
        raise ValueError("ERROR base passed to mut(): " +
                         str(base) + " not one of (A,T,C,G)\n")
...
def makemut(args, hc, avoid, alignopts):
    mutid_list = []
    for site in hc:
        mutid_list.append(site['chrom'] + '_' + str(site['start']) + '_' + str(
            site['end']) + '_' + str(site['vaf']) + '_' + str(site['altbase']))

    if args.seed is not None:
        random.seed(int(args.seed) + int(hc[0]['start']))

    bamfile = pysam.AlignmentFile(
        args.bamFileName, reference_filename=args.refFasta)
    # use for mates to avoid iterator problems
    ...
    for site in hc:
        ...
        if vaf is None:
            ...
            logger.warning(
                "multiple VAFs for single haplotype, using first encountered VAF: %f" % vaf)

        # position of mutation in genome
        mutpos = int(random.uniform(site['start'], site['end']+1))
    ...
    for n, mutpos in enumerate(mutpos_list):
        ...
        if altbase == refbase.upper() and not args.ignoreref:
            logger.warning(
                "%s specified ALT base matches reference, skipping mutation" % mutid_list[n])
            return None
        ...
        try:
            ...
            logger.warning(mutid_list[n] + " " + ' '.join(("skipped site:", chrom, str(
                hc[n]['start']), str(hc[n]['end']), "due to N base:", str(e), "\n")))
            return None
    ...
    hapstr = "_".join(('haplo', chrom, str(
        min(mutpos_list)), str(max(mutpos_list))))
    log = open('addsnv_logs_' + os.path.basename(args.outBamFile) + '/' +
               os.path.basename(args.outBamFile) + "." + hapstr + ".log", 'w')

    tmpoutbamname = args.tmpdir + "/" + hapstr + \
        ".tmpbam." + str(uuid4()) + ".bam"
    logger.info("%s creating tmp bam: %s" % (hapstr, tmpoutbamname))
    outbam_muts = pysam.AlignmentFile(tmpoutbamname, 'wb', template=bamfile)

    mutfail, hasSNP, maxfrac, outreads, mutreads, mutmates = mutation.mutate(args, log, bamfile, bammate, chrom, min(mutpos_list), max(
        mutpos_list)+1, mutpos_list, avoid=avoid, mutid_list=mutid_list, is_snv=True, mutbase_list=mutbase_list, reffile=reffile)
    ...
    if cnv:  # cnv file is present
        if chrom in cnv.contigs:
            for cnregion in cnv.fetch(chrom, min(mutpos_list), max(mutpos_list)+1):
                # expect chrom,start,end,CN
                cn = float(cnregion.strip().split()[3])
                logger.info(hapstr + "\t" + ' '.join(("copy number in snp region:",
                            chrom, str(min(mutpos_list)), str(max(mutpos_list)), "=", str(cn))))
                if float(cn) > 0.0:
                    vaf = vaf/float(cn)
                else:
                    vaf = 0.0
    ...
    if lastread < int(args.minmutreads):
        if len(readlist) > int(args.minmutreads):
            ...
            logger.warning(
                "%s dropped site with fewer reads than --minmutreads" % hapstr)
            os.remove(tmpoutbamname)
            return None
    elif lastread <= 0:
    ...
    for extqname, read in outreads.items():
        if read.seq != mutreads[extqname]:
            if not args.nomut and extqname in readlist:
                # changing seq resets qual (see pysam API docs)
                qual = read.qual
    ...
    if not hasSNP or args.force:
        ...
        remap_bam(args.aligner, tmpoutbamname, args.refFasta, alignopts, threads=int(
            args.alignerthreads), mutid=hapstr, paired=(not args.single), picardjar=args.picardjar)

        outbam_muts = pysam.AlignmentFile(
            tmpoutbamname, reference_filename=args.refFasta)
        coverwindow = 1

        avgincover = get_avg_coverage(bamfile, chrom, min(
            mutpos_list)-coverwindow, max(mutpos_list)+coverwindow)
        avgoutcover = get_avg_coverage(outbam_muts, chrom, min(
            mutpos_list)-coverwindow, max(mutpos_list)+coverwindow)
        ...
        if (avgoutcover > 0 and avgincover > 0 and avgoutcover/avgincover >= float(args.coverdiff)) or args.force:
            ...
            for n, site in enumerate(hc):
                snvstr = chrom + ":" + \
                    str(site['start']) + "-" + str(site['end']) + \
                    " (VAF=" + str(vaf) + ")"
                log.write("\t".join(("snv", snvstr, str(mutpos_list[n]), mutstr_list[n], str(
                    avgincover), str(avgoutcover), str(spikein_snvfrac), str(maxfrac)))+"\n")
        else:
...
def main(args):
    ...
    if (args.bamFileName.endswith('.bam') and not os.path.exists(args.bamFileName + '.bai')) or \
        ...
        logger.error(
            "input file must be indexed, not .bai or .crai file found for %s" % args.bamFileName)
        sys.exit(1)
    ...
    for bedline in bedfile:
        if ntried < int(args.numsnvs) or int(args.numsnvs) == 0:
            ...
            if len(c) == 5:
                altbase = c[4].upper()
                assert altbase in [
                    'A', 'T', 'C', 'G'], "ERROR: ALT " + altbase + " not A, T, C, or G!\n"
                target['altbase'] = altbase

            targets.append(target)
            ntried += 1

    targets = sorted(targets, key=itemgetter('chrom', 'start')
                     )  # sort list of dicts by chrom, start
    ...
    if args.skipmerge:
        logger.info(
            "skipping merge, plase merge reads from %s manually." % outbam_mutsfile)
    else:
        if args.tagreads:
        ...
        rr.replace_reads(args.bamFileName, outbam_mutsfile,
                         args.outBamFile, args.refFasta, keepqual=True, seed=args.seed)
    ...
    makevcf.write_vcf_snv(
        'addsnv_logs_' + os.path.basename(args.outBamFile), args.refFasta, vcf_fn)

    logger.info('vcf output written to ' + vcf_fn)

    try:
        donefile = args.outBamFile + '.complete'
        open(donefile, 'w').close()
        logger.info("Wrote completion sentinel: %s" % donefile)
    except Exception as e:
        logger.warning("Could not write completion sentinel: %s" % str(e))


def run():
    parser = argparse.ArgumentParser(
        description='adds SNVs to reads, outputs modified reads as .bam along with mates')
    parser.add_argument('-v', '--varfile', dest='varFileName',
                        required=True, help='Target regions to try and add a SNV, as BED')
    parser.add_argument('-f', '--bamfile', dest='bamFileName',
                        required=True, help='sam/bam file from which to obtain reads')
    ...
    parser.add_argument('-o', '--outbam', dest='outBamFile',
                        required=True, help='.bam file name for output')
    ...
    parser.add_argument('-n', '--numsnvs', dest='numsnvs', default=0,
                        help="maximum number of mutations to try (default: entire input)")
    ...
    parser.add_argument('-d', '--coverdiff', dest='coverdiff', default=0.9,
                        help="allow difference in input and output coverage (default=0.9)")
    parser.add_argument('-z', '--haplosize', default=0,
    ...
    parser.add_argument('--picardjar', required=True,
                        help='path to picard.jar, required for most aligners')
    parser.add_argument('--mindepth', default=10,
                        help='minimum read depth to make mutation (default = 10)')
    parser.add_argument('--maxdepth', default=2000,
                        help='maximum read depth to make mutation (default = 2000)')
    parser.add_argument('--minmutreads', default=3,
                        help='minimum number of mutated reads to output per site')
    parser.add_argument('--avoidreads', default=None,
                        help='file of read names to avoid (mutations will be skipped if overlap)')
    parser.add_argument('--nomut', action='store_true',
                        default=False, help="dry run")
    parser.add_argument('--ignoresnps', action='store_true', default=False,
                        help="make mutations even if there are non-reference alleles sharing the relevant reads")
    parser.add_argument('--ignoreref', action='store_true', default=False,
                        help="make mutations even if the mutation is back to the reference allele")
    parser.add_argument('--force', action='store_true', default=False,
                        help="force mutation to happen regardless of nearby SNP or low coverage")
    parser.add_argument('--insane', action='store_true', default=False,
                        help="ignore sanity check enforcing input read count = output read count in realignment")
    parser.add_argument('--single', action='store_true', default=False,
                        help="input BAM is simgle-ended (default is paired-end)")
    parser.add_argument('--maxopen', dest='maxopen', default=1000,
                        help="maximum number of open files during merge (default 1000)")
    parser.add_argument('--requirepaired', action='store_true',
                        default=False, help='skip mutations if unpaired reads are present')
    parser.add_argument('--tagreads', action='store_true',
                        default=False, help='add BS tag to altered reads')
    parser.add_argument('--skipmerge', action='store_true',
                        default=False, help="final output is tmp file to be merged")
    ...
    parser.add_argument('--tmpdir', default='addsnv.tmp',
                        help='temporary directory (default=addsnv.tmp)')
    parser.add_argument('--seed', default=None,
                        help='seed random number generation')
    parser.add_argument(
        '--vcf', default='', help="Path for the output VCF file. If not provided, the file will be saved in the current directory.")
